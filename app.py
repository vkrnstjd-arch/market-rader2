...
def load_google_portfolio(sheet_url: str, sheet_name: str):
    """
    Public Google Sheet loader using Streamlit's maintained GSheetsConnection.
    No service account is required because the sheet is shared as 'anyone with the link: viewer'.
    We disable result caching here so portfolio edits show up immediately after a rerun.
    """
    if not re.search(r"/spreadsheets/d/([a-zA-Z0-9_-]+)", sheet_url):
        raise ValueError("Google Sheet URL에서 문서 ID를 찾지 못했습니다.")

    conn = st.connection("portfolio_gsheets", type=GSheetsConnection)
    try:
        raw = conn.read(
        ...
        raise RuntimeError(f"GSheetsConnection 읽기 실패: {type(e).__name__}: {e}") from e

    if raw is None or len(raw) == 0:
        raise ValueError("Google Sheet에서 빈 데이터가 반환되었습니다.")

    raw = raw.copy().reset_index(drop=True)
    raw = raw.astype(object).where(pd.notna(raw), "")

    # Some connection versions may still infer the first row as column names.
    # Put column names back as row 0 when needed so header detection is invariant.
    def _contains_headers(frame):
        for i in range(min(len(frame), 20)):
            vals = [str(x).strip() for x in frame.iloc[i].tolist()]
            if "종목" in vals and "비중" in vals:
                return True
        return False

    if not _contains_headers(raw):
        colrow = [str(c).strip() for c in raw.columns]
        raw2 = pd.concat([pd.DataFrame([colrow]), raw], ignore_index=True)
        if _contains_headers(raw2):
            raw = raw2
    ...
    if header_row is None:
        preview = raw.head(8).astype(str).to_csv(index=False)[:1200]
        raise ValueError(f"'종목'과 '비중' 헤더를 찾지 못했습니다. 상단 미리보기: {preview}")
    ...
    for _, row in body.iterrows():
        ...
        if not name or name.lower() == "nan":
            continue
        # 'Pluto' marks the start of the watchlist / scratch area. Never read below it as holdings.
    ...
    total = df["평가금액"].sum()
    df["비중"] = df["평가금액"] / total * 100
    df["데이터원"] = f"Google Sheet/{sheet_name} · LIVE"
    return df
...
- Google Sheet 포트는 매 rerun 시 LIVE 재조회 (GSheetsConnection ttl=0)
...
# UI — LEFT NAVIGATION
# =========================================================
BUILD_VERSION = "v13.0 · GSheets LIVE + asset-specific crash depth"
...
if portfolio_error:
    st.warning(f"Google Sheet LIVE 읽기 실패 → 최신 내장 fallback 포트를 사용 중입니다. 원인: {portfolio_error}")
else:
